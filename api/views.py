...
def barangay(request, barangay_name):
    ...
    occurrences = Crime.objects.filter(barangay=chosen_barangay)
    ...
    for occurrence in occurrences.values("name", "datetime"):
        # Get datetime of current crime occurrence
        current_year = occurrence["datetime"].year
        current_crime = occurrence["name"]

        # Append yearly totals
        if current_year not in data["yearly"]:
            data["yearly"][current_year] = {}
        
        if current_crime not in data["yearly"][current_year]:
            data["yearly"][current_year][current_crime] = 1
        elif current_crime in data["yearly"][current_year]:
            data["yearly"][current_year][current_crime] += 1
