...
function DataVisualizer(props) {
  ...
  const [cIsLatest, setCIsLatest] = useState(false);
  ...
  const [bIsLatest, setBIsLatest] = useState(false);
  ...
  if (props.dataType === 'barangay') {
    ...
    function updateChart(year) {
      ...
      if (year) {
        Object.entries(props.barangayData.yearly[year]).forEach(data => {
          data = data[0]
      ...
      if (year) {
        setBData(Object.values(props.barangayData.yearly[year]));
      } else {
        setBData([]);
      }
    }
    ...
    for (const key in props.barangayData.yearly) {
      let dtString = new Date(key);
    ...
    return(
      <div>
        ...
        <ChartDropdown year={bYear} years={years} handleChartDropdownChange={handleChartDropdownChange} />
  ...
  // Display a data visualizer for barangay and crime data
    ...
    function updateChart(year) {
      ...
      setBCMax(props.barangayCrimeData['yearly_totals'][year])
    ...
    plugins: {
      ...
      datalabels: {
        ...
        formatter: function(value, ctx) {
          ...
          const string = `${ctx.chart.data.labels[ctx.dataIndex]} - ${percentage}%`;
